...
public sealed class RegisterValidator : AbstractValidator<RegisterDto>
{
    ...
    {
        RuleFor(p => p.FirstName).NotEmpty().WithMessage("Geçerli bir kullanıcı adı ya da email adresi girin!");
        RuleFor(p => p.FirstName).MinimumLength(3).WithMessage("Geçerli bir kullanıcı adı ya da email adresi girin!");

        RuleFor(p => p.LastName).NotEmpty().WithMessage("Geçerli bir soyadı girin!");
        RuleFor(p => p.LastName).MinimumLength(3).WithMessage("Geçerli bir soyadı girin!");
